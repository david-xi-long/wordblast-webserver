...
export default function ChatRoom(props: {gameSocket: GameSocket; username: string; gameId: string}) {
    ...
    const receiveMessages = () => {
        props.gameSocket.subscribe<PacketInPlayerMessage>('chat-message', (packet: { getMessage: () => any; getUsername: () => any; }) => {
            console.log(packet.getMessage(), packet.getUsername());
            setMessages(oldMessages => [...oldMessages, packet.getMessage()]);
        });
    };
    ...
    const sendMessage = async(e: KeyboardEvent<HTMLInputElement>) => {
        if (e.keyCode === 13) {
            ...
            props.gameSocket.fireAndForget("chat-message", new PacketOutPlayerMessage(props.gameId, props.username, formValue));
            setFormValue('');
        }
    }
